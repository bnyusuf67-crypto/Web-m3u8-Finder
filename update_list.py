...
import json
import traceback
import time  # Bekleme yapabilmek için time modülü eklendi

MAX_RETRIES = 3    # Hata durumunda kaç kez tekrar deneneceği
RETRY_DELAY = 5    # Her deneme arasında kaç saniye bekleneceği
...
def main():
    ...
    for idx, channel in enumerate(channels, 1):
        ...
        # --- RE-TRY (YENİDEN DENEME) DÖNGÜSÜ ---
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                if attempt > 1:
                    print(f"  🔄 Retrying... Attempt {attempt}/{MAX_RETRIES}")
                
                # Streamlink üzerinden akışları çekme
                streams = streamlink.streams(url)
                
                # Başarılı şekilde çekildi ve "best" akışı var ise döngüden çık
                if streams and 'best' in streams:
                    break
                else:
                    print(f"  ⚠️ Attempt {attempt}/{MAX_RETRIES}: Stream or 'best' quality not found.")
            except Exception as e:
                print(f"  ⚠️ Attempt {attempt}/{MAX_RETRIES} failed with error: {e}")
            
            # Son deneme değilse tekrar denemeden önce bekle
            if attempt < MAX_RETRIES:
                time.sleep(RETRY_DELAY)

        # Denemeler tamamlandıktan sonra akış durumunu kontrol et
        try:
            if not streams or 'best' not in streams:
                print(f"  ❌ Failed to retrieve valid streams for {slug} after {MAX_RETRIES} attempts.")
                fail_count += 1
                
                # Eski hatalı/artık dosyaları temizleme
